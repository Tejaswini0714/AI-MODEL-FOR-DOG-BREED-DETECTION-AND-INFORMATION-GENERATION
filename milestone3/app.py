...
# ---------------------------- CHECK FOR "page=signup" LINK CLICK ----------------------------
...
if "page" in query_params:
    ...
    if page_param != st.session_state.get("page", "home"):
...
if st.session_state.page == "home":
    st.markdown("<h1 style='text-align: center; margin-top:8px;'>🐶 Dog Breed Detection and Information Generation</h1>", unsafe_allow_html=True)
        ...
        with st.form("login_form"):
            ...
            st.markdown("""
                <div style='text-align: center; font-size: 15px;'>
                    ...
                    <a href='?page=signup' style='color: red; font-weight: bold; text-decoration: none;'>Sign Up</a>
                </div>
                """, unsafe_allow_html=True)
...
# ---------------------------- MAIN APP ----------------------------
   ...
   # ---------------- History Page ----------------
        ...
        st.markdown("<h1 style='text-align:center; margin-bottom: 20px;'>📜 Prediction History</h1>", unsafe_allow_html=True)
        ...
        if not history:
            ...
            for h in history:
                ...
                created_at_raw = ensure_str(h[4])
                try:
                    # Parse the original UTC datetime
                    dt_utc = datetime.strptime(created_at_raw, "%Y-%m-%d %H:%M:%S")
                    dt_utc = pytz.utc.localize(dt_utc)  # mark it as UTC

                    # Convert to IST
                    dt_ist = dt_utc.astimezone(ist)

                    # Format as day-month-year and hour:minute:second
                    created_at = f"{dt_ist.day}-{dt_ist.month}-{dt_ist.year} {dt_ist.hour:02d}:{dt_ist.minute:02d}:{dt_ist.second:02d}"
                except Exception:
                ...
                if os.path.exists(img_path):
                    try:
                        ...
                        img_html = f'<img src="data:image/jpeg;base64,{img_base64}" style="width:200px; height:200px; object-fit:cover; border-radius:10px;" />'
                    except Exception:
                        img_html = "<div style='color:red;'>Image not available.</div>"
                else:
                    img_html = "<div style='color:red;'>Image not found.</div>"

                # Render horizontal card with centered text
                st.markdown(f"""
                <div style="
                    background-color: white;
                    border-radius: 15px;
    ...
    # ---------------- Main Dog Breed Detector ----------------
        ...
        st.markdown("<h1 style='text-align: center;'>🐶 Dog Breed Detection and Information Generation</h1>", unsafe_allow_html=True)
        st.markdown("<div class='center-column'>", unsafe_allow_html=True)

        uploaded_file = st.file_uploader("Upload a dog image", type=["jpg", "jpeg", "png"], label_visibility="visible")

        upload_btn_clicked = False
        if uploaded_file:
    # Inject CSS to style the real Streamlit button
            st.markdown("""
                <style>
                div.stButton > button:first-child {
                    ...
                    margin-left: 40px; /* slight right shift */
                }
            ...
            # Centered and styled functional button
            col1, col2, col3 = st.columns([1, 1, 1])
            with col2:
                if st.button("Upload 🐶", key="upload_img_btn"):
                    upload_btn_clicked = True


        if uploaded_file and upload_btn_clicked:
            ...
            if model is None:
                ...
                img.save(img_path)
                add_history(st.session_state.user_id, chosen_breed, float(confidence), img_path)
